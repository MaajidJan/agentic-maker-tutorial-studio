...
> Autonomous Multimodal AI Agent Studio transforming robotics & electronics media into thrilling, kid-friendly (~8-12yo) tutorial video packages with timestamped scripts, anime reaction stickers, and sound effects.
...
7. **Google Cloud Infrastructure Readiness (Cloud Run + Firestore)**:
...
<p align="center">
  <img src="docs/workflow_diagram.svg" width="100%" alt="Autonomous Agentic Maker Studio Workflow Lifecycle" />
</p>

## 🚀 Quick Start & Local Execution

### 1. Prerequisites & Environment Setup
Make sure you have Python 3.10+ installed and the environment variables configured in your `.env`:
```bash
...
python cli.py --media sample_data/arduino_radar.jpg --prompt "Make this ultrasonic radar build exciting for 10-year-olds with anime reactions and cool sound effects!"

# Non-interactive automated mode:
python cli.py --auto-approve
```
...
Follow these exact `gcloud` commands to deploy as a managed Cloud Run service with Firestore persistence:
...
```bash
gcloud run deploy agentic-maker-tutorial-studio \
    --source . \
    --region us-central1 \
    --platform managed \
    --allow-unauthenticated \
    --set-env-vars "GEMINI_API_KEY=$GEMINI_API_KEY,GEMINI_MODEL=gemini-3.5-flash,GOOGLE_CLOUD_PROJECT=$GCP_PROJECT,USE_FIRESTORE=true,FIRESTORE_COLLECTION=maker_tutorial_jobs" \
    --memory 2Gi \
    --cpu 2 \
    --timeout 300
```
...
```text
Service URL: https://agentic-maker-tutorial-studio-xxxxx-uc.a.run.app
```
